...
from PyQt6.QtWidgets import QTabWidget, QTabBar, QStyleFactory
...
class TabWidget(QTabWidget):
    ...
    def _getTextSize(self, index, orig_text):
        if self.tabPosition() == QTabWidget.TabPosition.North:
            ...
            b_width = 0 if button is None else (button.width() + 9)  # add padding too (empyrical)
            available_width = max(0, self.width() - ((self.icon_size + b_width) * self.count()))
            label_width = available_width / self.count()
            if label_width < DefaultSettings.Tabs.maxWidth - (self.icon_size + b_width):
